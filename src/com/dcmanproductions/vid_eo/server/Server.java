...
import com.dcmanproductions.vid_eo.Audio.ServerStream;
...
public class Server implements Runnable {
		...
		block2: while (this.running) {
			...
			if (text.equals("clients")) {
				...
				while (i < this.clients.size()) {
					...
					System.out.println(
							String.valueOf(c.name) + "(" + c.getID() + "): " + c.address.toString() + ":" + c.port);
					++i;
				}
				System.out.println("========");
				continue;
	...
	private void disconnect(int id, boolean status) {
		...
		String message = "";
		message = status
				? "Client " + c.name + " (" + c.getID() + ") @ " + c.address.toString() + ":" + c.port
						+ " disconnected."
				: "Client " + c.name + " (" + c.getID() + ") @ " + c.address.toString() + ":" + c.port + " timed out.";
		System.out.println(message);
	}
